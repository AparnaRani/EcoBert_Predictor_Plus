# run_gpt2_xl_exp_3.py (Designed for a longer, higher-emission run)
import os
import uuid
import pandas as pd
import torch
...
if __name__ == '__main__':
    # --- PARAMETERS FOR THIS SPECIFIC GPT-2 XL RUN (Experiment 3) ---
    # These parameters are chosen to generate significantly higher emissions.
    current_experiment_params = {
        ...
        # Designed for a longer, higher-emission run
        'num_train_samples': 25000,         # Full dataset size
        'num_epochs': 10,                   # SIGNIFICANTLY MORE EPOCHS to increase duration
        'batch_size': 4,                    # Smaller batch size to prevent OOM on P100 with many epochs
        'gradient_accumulation_steps': 4,   # Higher accumulation for effective batch size of 16 (4*4)
    }
